...
{
    if(!sender()) return 0;
    return static_cast<QTcpSocket*>(sender()); //opisac dokladnie co tu sie dizeje nie do konca zrozumiale z tutoriala
}
...
{
    if(!sender()) return;
    qDebug() << this << "stateChanges" << getSocket() << "states = " << socketState;
}

void TcpConnecton::error(QAbstractSocket::SocketError socketError)
{
    if(!sender()) return;
    qDebug() << this << "ERROR " << getSocket() << "errpr= " << socketError;
}
